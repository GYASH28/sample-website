...
const routes = [
  ...
  { path: "/compare", canonical: "/compare", indexable: false },
];
...
(async () => {
  ...
  try {
    for (const route of routes) {
      ...
      const state = await page.evaluate(() => {
        ...
        const jsonLd = [...document.querySelectorAll('script[type="application/ld+json"]')]
          ...
          .filter(Boolean);
        return { title: document.title, canonical, robots, description, h1Count: document.querySelectorAll("h1").length, jsonLd, text: document.body.innerText };
      });
      ...
      assert(state.description.length >= minimumDescription && state.description.length <= 210, `${route.path}: weak description length (${state.description.length})`);
      ...
      if (route.indexable) assert(normalizedRobots.includes("index") && !normalizedRobots.includes("noindex"), `${route.path}: should be indexable (${state.robots})`);
      else assert(normalizedRobots.includes("noindex"), `${route.path}: utility route must be noindex (${state.robots})`);
      ...
      if (route.path.startsWith("/products/")) {
        ...
        assert(Array.isArray(itemPage.hasPart) && itemPage.hasPart.length > 0, "product: published catalogue options should be represented as page parts");
      }

      const serialized = JSON.stringify(state.jsonLd);
      assert(!serialized.includes('"@type":"Offer"'), `${route.path}: unsupported Offer schema present`);
      assert(!serialized.includes("aggregateRating"), `${route.path}: unsupported aggregateRating present`);
      assert(!serialized.includes("reviewCount"), `${route.path}: unsupported reviewCount present`);
      assert(!serialized.includes("InStock") && !serialized.includes("OutOfStock"), `${route.path}: unsupported live availability schema present`);
    ...
    for (const expected of [
      ...
      "/collections/macrame-cord",
      "/collections/embroidery-thread",
      "/collections/yarn-for-bags",
      "/collections/cotton-yarn",
      "/collections/wholesale-yarn-pune",
      "/collections/yarn-craft-supplies-pune",
    ]) {
      assert(sitemap.includes(`${SITE_URL}${expected}`), `sitemap: missing ${expected}`);
    }
    for (const utility of ["/wishlist", "/enquiry", "/compare"]) {
      assert(!sitemap.includes(`${SITE_URL}${utility}`), `sitemap: ${utility} utility route should be excluded`);
    }
