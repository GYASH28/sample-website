...
function Counter({ value }) {
  return value > 0 ? <span key={value} className="nav-count" aria-hidden="true">{value}</span> : null;
}
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      const focusable = drawerRef.current?.querySelectorAll('a, button, input, [tabindex]:not([tabindex="-1"])');
  ...
  return (
    <>
      <header className="site-header">
        ...
        <div className="container nav-shell">
          ...
          <nav className="desktop-nav" aria-label="Primary navigation">
            ...
            {primaryLinks.slice(1).map((item) => <NavLink key={item.to} to={item.to}>{t(item.key)}</NavLink>)}
          </nav>
      ...
      <button className={`drawer-backdrop ${menuOpen ? "is-open" : ""}`} type="button" onClick={closeMenu} aria-label="Close navigation" aria-hidden={!menuOpen} disabled={!menuOpen} tabIndex={menuOpen ? 0 : -1} />
      <aside ref={drawerRef} className={`mobile-nav-drawer ${menuOpen ? "is-open" : ""}`} role="dialog" aria-modal="true" aria-label="Mobile navigation" aria-hidden={!menuOpen} inert={!menuOpen}>
